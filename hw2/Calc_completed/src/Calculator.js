...
class Calculator extends React.Component {
  ...
  evaluate = () => {
    ...
    // console.log("previousEvaluations: " + previousEvaluations)
    if(previousEvaluations.length !== 0)
    {
      // Get last num
      let parts = previousEvaluations[previousEvaluations.length-1].split('=');
      let leftSide = parts[0]; // Получим '5+3+6 '

      // Используем регулярное выражение для поиска всех чисел в строке
      let numbers = leftSide.match(/\d+/g);

      // Возьмём последнее число
      let lastNumber = numbers[numbers.length - 1];

      console.log(lastNumber);
      evaluation = (evaluation + this.state.lastOperation + lastNumber)
